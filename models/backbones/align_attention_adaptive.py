...
class AlignAttention(nn.Module):
    ...
    """
    def __init__(self, hidden_size, inner_size=None, num_patch=25, feats_size=5, drop_prob=0., shot=1, keep_rate=0.5):
    ...
    def compute_distances(self, query_a, key_a, value_a, query_b, key_b, value_b, features_a, features_b):
        ...
        # Reconstructed features B
        att_scores = torch.matmul(global_b.unsqueeze(-2).unsqueeze(-2), value_b.transpose(-2, -1))
        att_probs = nn.Softmax(dim=-1)(att_scores / math.sqrt(self.inner_size))
        threshold_value = torch.quantile(att_probs, q=(1.-self.keep_rate), dim=-1, keepdim=True, interpolation='higher')
        att_probs = torch.relu((att_probs - threshold_value))

        # (N_query x N_way x 1 x HW x N-shot*HW) x (1 x N_way x 1 x N-shot*HW x C) -> (N_query x N_way x 1 x HW x C)
        bar_q = torch.matmul(att_probs, value_b)


        # Reconstructed features B
        att_scores = torch.matmul(bar_q, reconstructed_features_b.transpose(-2, -1))
        att_probs = nn.Softmax(dim=-1)(att_scores / math.sqrt(self.inner_size))
        threshold_value = torch.quantile(att_probs, q=(1.-self.keep_rate), dim=-1, keepdim=True, interpolation='higher')
        att_probs = torch.relu((att_probs - threshold_value))

        # (N_query x N_way x 1 x HW x N-shot*HW) x (1 x N_way x 1 x N-shot*HW x C) -> (N_query x N_way x 1 x HW x C)
        agg_recon_b = torch.matmul(att_probs, reconstructed_features_b)


        scores_filter = agg_recon_b@bar_q.transpose(-2, -1)
        scores_filter = scores_filter.squeeze()

        reconstructed_features_b_mean = reconstructed_features_b.mean(dim=-2)
        value_b_mean = value_b.mean(dim=-2)
        scores_align = reconstructed_features_b_mean@value_b_mean.transpose(-2, -1)
        scores_align = scores_align.squeeze()
